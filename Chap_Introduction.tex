...
Attributes in this document are organized according to their primary usage, either grouped with a specific \ac{API} or included in an appropriate functional chapter. \ac{PMIx} Standard attributes all start with \code{"PMIX"} in their name, and many include a functional description as part of their name (e.g., the use of "PMIX_FABRIC_" at the beginning of fabric-specific attributes). The Standard also defines one generalized attribute for a special purpose:
