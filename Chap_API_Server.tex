...
The largest time costs during application startup have historically been the time required to collectively share connection endpoint information, followed by the barrier prior to commencing operations. Analysis
of exchanged data revealed that much of the information in the exchange was known to the host environment at the time it started an application process, but that the host had no mechanism for communicating that information to the client process. The remainder was dynamically-assigned endpoint connections that client processes independently discovered and then contributed to the collective operation.

With this in mind, \ac{PMIx} approaches the launch sequence with a goal of completely eliminating both the exchange of endpoints and the subsequent global barrier, regardless of application scale. In addition, the community determined that the file system retrieval bottleneck could be effectively eliminated by pre-positioning the required information. The revised launch sequence consists of the following stages, as shown in Fig. 3 (steps in red indicate operations implemented in v2.1 of the PMIx reference library):
