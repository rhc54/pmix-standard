...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Supporting large-scale launch}
\label{chap:api_init:launcsupport}


\subsection{Instant On}
\label{chap:server:instanton}

As previously discussed, the largest time costs during application startup have historically been the time required to collectively share connection endpoint information (BCX), followed by the barrier prior to commencing operations. Analysis
 16 R.H. Castain et al./Parallel Computing 79 (2018) 9–29
Fig. 3. PMIx-enabled launch.
of exchanged data revealed that over 90\% of the information in the BCX was known to the local RM daemon at the time it started an application process, but that the daemon had no mechanism for communicating that information to the client process. The remainder was dynamically-assigned endpoint connections that client processes independently discovered and then contributed to the collective operation.
With this in mind, the PMIx community approached the launch sequence with a goal of completely eliminating both the BCX and the subsequent global barrier, regardless of application scale. In addition, the community determined that the file system retrieval bottleneck could be effectively eliminated by pre-positioning the required information. The revised launch sequence consists of the following stages, as shown in Fig. 3 (steps in red indicate operations implemented in v2.1 of the PMIx reference library):

\subsubsection{Collecting inventory}
Stage 0. While not technically part of the launch procedure, the inventory of resources (including NICs, switches, network and node topology, memory and CPUs, etc) is critical for scheduling applications as well as efficient completion of the launch process. Given the broad informational needs of the revised launch process, and the role of PMIx as cross-system integrator, he PMIx community was asked to provide a dynamic way of assembling the inventory and updating it as changes occurred. Accordingly, the community has provided three mechanisms for inventory support:
• rollup of inventory at time of boot. The PMIx_server_collect_inventory API was added to request that PMIx sample local topology and return the information to the host daemon for rollup to the WLM. This is, of course, limited to whatever information the PMIx implementation supports. The PMIx reference library currently uses hwloc [19] for this purpose, thereby providing topological information on available CPUs, memory, GPUs, and network interfaces. Additional support will be added as needs are identified;
• querying of SMS subsystems upon request. The information provided during rollup is only local — i.e., PMIx will report on the resources located on its node. WLMs and other inventory consumers also need to understand the system ele- ments that knit the nodes together — e.g., the identity of switches and their connection patterns, plus the node-to-node transit time for messages. This information is best obtained by directly querying the respective subsystem. For example, information on the network is obtained via request from the system fabric manager and then committed to the PMIx key-value store on the WLMs node; and
• updates via PMIx event notification. While infrequent, inventory information can change over time either due to equip- ment replacement or temporary outages (e.g., network bottlenecks). Continuously polling for information places an un- due strain on the system. Therefore, the PMIx community is working with subsystem providers to obtain notification of changes either directly furnished as PMIx events, or translated by a PMIx server into such events. These events are subsequently used to update the key-value store and alert registered parties to the change.
This information is provided to the WLM (for use in scheduling operations) as well as any other interested party via the PMIx_Query API. Local resource info (e.g., memory and CPU topology) is also cached in shared memory on each node for later availability to applications.

\subsubsection{Scheduling support}

Stage 1. User submits a job (either in the form of a script or a request for an interactive session) to the WLM. The request typically will include descriptions of the type and amount of resources required for the job. In addition to logging the request into the appropriate queue, the WLM uses a PMIx interface to pass a request to the file system for estimated retrieval times of all user-identified executables, supporting libraries, and data files. The resulting estimates can then be factored into the scheduling algorithm — this is especially important as hot/warm/cold storage policies become more common.

R.H. Castain et al./Parallel Computing 79 (2018) 9–29 17
The burden on the user to identify all required libraries is often an issue. Hence, PMIx provides another interface by which the WLM can first request that a job script be analyzed to return a list of associated libraries (e.g., the equivalent to the output of a standard POSIX ldd command) that can be included in the FS request for retrieval time. It is expected that the development of increasingly accurate analyzers will become a research area of interest.

Include description of the fabric and other interfaces supporting scheduling

\subsubsection{Execution preparation}

Stage 2. Depending upon resource availability, relative priorities, and scheduling algorithm, at some point in the future the WLM will assign resources to the requested job. This assignment typically occurs when the WLM is notified of job completion and just prior to initiation of the epilogue portion of the prior job, but many WLMs also utilize anticipatory scheduling to identify one or more jobs that will be run under different termination scenarios. In addition, the need to aggregate resources for larger jobs often results in at least some portion of the allocation becoming available on a rolling basis. Thus, there frequently is some amount of time available to the SMS for pre-conditioning of the allocation prior to start of a newly scheduled job.
PMIx takes advantage of this time to accomplish two important steps. First, it provides an interface by which the WLM can notify the FS of the allocation and pass it the list of required files as determined during the initial stage. This allows appropriately enabled file systems to pre-cache the files to a location network-near the allocated compute nodes, assuming such caching elements are available. Provided the file system support network is separate from the compute fabric, caching operations can execute in the background based on anticipated allocation locations with little to no impact on currently running applications. The file system generates a PMIx event (monitored by the WLM) once caching is complete.
Second, the WLM can utilize another PMIx interface to pass both the allocation and job description to subsystems such as the fabric manager, receiving back a “blob” containing all information each subsystem has determined will be required by its local resources within the allocation to support the described job. Returned values are marked to identify which are to be given directly to the local resource (e.g., network driver) on each node, and which are to be passed to application processes as environmental variables that are recognized by the resource when initialized by that specific process. For example, some networks may “preload” the local network library with the location of all processes in that application, thus allowing the library to compute the required address information for any process, and/or include a security token to protect inter-process communications.

\subsubsection{}
Stage 3. Once the required files have been cached and the network configuration obtained, the WLM passes the initial- ization “blob” along with the job description and related inventory information for that allocation (e.g., relative network route times between allocated nodes for network-aware collectives) to the RM daemons on the allocated nodes. Prior to spawning any local processes, each RM daemon can utilize a PMIx interface to deliver the configuration information to the local resources. This allows the resources to prepare any internal state tables for the job, an operation which may require elevated privileges and therefore cannot be performed by the application processes themselves. In addition, the RM daemon registers the application and local processes with the local PMIx server, providing it with all relevant information pertaining to the application and local environment. The PMIx community has assembled a list of desired information to assist SMS vendors in meeting this requirement [20]. With this accomplished, the RM daemon spawns the application processes.
...
\begin{itemize}
    \item \pasteAttributeItem{PMIX_SERVER_NSPACE}
    \item \pasteAttributeItem{PMIX_SERVER_RANK}
    \item \pasteAttributeItem{PMIX_NSPACE}
...
Application-level information is accessed by providing both the namespace of the job (with the rank set to \refconst{PMIX_RANK_WILDCARD}). If application-level information is requested for an application other than the one the caller belongs to, then the \refattr{PMIX_APPNUM} attribute must be provided. If more than one application is included in the namespace, then the the host environment is also required to supply data consisting of the following items for each application in the job, passed as a \refstruct{pmix_data_array_t} using the \refattr{PMIX_APP_INFO_ARRAY} attribute:

\begin{itemize}
    \item \pasteAttributeItem{PMIX_APPNUM}
...
%%%%%%%%%%%
\subsection{\code{PMIx_generate_locality_string}}
\declareapi{PMIx_generate_locality_string}
...
\begin{codepar}
pmix_status_t
PMIx_generate_locality_string(const pmix_cpuset_t *cpuset,
                              char **locality);
\end{codepar}
\cspecificend
...
\subsection{Cpuset support macros}
The following macros support the \refstruct{pmix_cpuset_t} structure.

\subsubsection{Initialize the cpuset structure}
...
Notify the host server that a client connected to this server. This function module entry has been DEPRECATED in favor of \refapi{pmix_server_client_connected2_fn_t}.
...
\subsubsection{Dmodex required key}
